...
if (nen==4)
    elemType='Q4';
    C=textscan(fid,'%u16 %u16 %u16 %u16','MultipleDelimsAsOne', 1);
    element=cell2mat(C(1:4));
elseif (nen==9)
    elemType='Q9';
    C=textscan(fid,'%u16 %u16 %u16 %u16 %u16 %u16 %u16 %u16 %u16','MultipleDelimsAsOne', 1);
    element=cell2mat(C(1:9));
elseif (nen==8)
    elemType='Q8';
    C=textscan(fid,'%u16 %u16 %u16 %u16 %u16 %u16 %u16 %u16','MultipleDelimsAsOne', 1);
...
disp_BC=[C{1} C{2} z];
C=textscan(fid,'%s',4);
C=textscan(fid,'%u16','MultipleDelimsAsOne', 1);
Force_Node=C{1};
C=textscan(fid,'%s',2);
C=textscan(fid,'%n',1);
bforce=C{1,1};
